/**
 * Startup guard: refuse to serve against a database whose schema was never
 * migrated.
 *
 ...
 * runs, so its absence is the exact signature of "booted against an empty
 * schema". The remaining entries are the oldest, most fundamental tables;
 * they catch a partially restored database without this list having to be
 * extended for every model added since.
 */
...
/**
 * Verify the schema is migrated, throwing an actionable error if it is not.
 *
 * Only "the relation is missing" is treated as a migration problem; anything
 ...
 */
export async function assertSchemaMigrated(probe: SchemaProbe): Promise<void> {
