...
Last verified: **2026-09-15** · Stellar **Testnet** · commit `3c7a6ae` plus the
uncommitted fixes in §8 (`#11`–`#20`)

> Re-audited 2026-09-15 against the working tree (not the commit history): the
> unit/e2e matrix and the live Testnet journey were **re-run** here, the
> transaction hashes quoted below were **re-checked against Horizon**, and three
> further defects were found and fixed (§8 `#18`–`#20`). Anything that could not
> be re-run in this environment is called out as such and left marked.
...
# 3c. failure injection (Postgres / Redis / Soroban RPC down, live)</nbash scripts/failure-injection.sh
...
| Suite | Result |
| ------------------------------- | ---------------------------------------------------------------------------------------------------- || `pnpm test` (8 projects) | **491 tests, 0 failures** (259 in gateway, 41 in dashboard across 5 suites) — re-run 2026-09-15 |
| `pnpm nx run gateway:test:e2e` | **51 tests, 0 failures** across 3 e2e suites (x402, notifications, escrow flows) — re-run 2026-09-15 |
| `pnpm exec nx run-many -t lint` | 0 errors (15 projects; 14 warnings, all `no-non-null-assertion`) — re-run 2026-09-15 |
| `pnpm exec prettier --check` | clean |
| `pnpm build` | clean |

These counts are the ones this repository actually produces; the earlier
`477 (245 in gateway)` was stale. Coverage thresholds are enforced by
`--coverage` in CI (gateway 70 % statements, SDK 80 %, validation 85 %, wallet
90 %; e2e 56 %).
...
### Failure injection against a live stack

`scripts/failure-injection.sh` boots the stack plus a second gateway whose
`SOROBAN_RPC_URL` points at a closed port, then stops PostgreSQL and Redis in
turn and probes every surface through each outage. The assertions are about
**safe failure**, not availability:

1. an unpaid request must never return `200`
2. a forged/unknown payment hash must never be accepted
3. an unverifiable escrow draw must be rejected, with a reason
4. readiness must name the dependency that is down

**Result: all invariants held in all four phases. 0 violations.** Evidence:
[`docs/evidence/failure-injection.json`](./evidence/failure-injection.json).

| Surface          | baseline | Postgres down | Redis down | Soroban RPC down |
| ---------------- | :------: | :-----------: | :--------: | :--------------: |
| `/health` (live) |   200    |      200      |    200     |       200        |
| `/health/ready`  |   200    |      503      |    503     |       200        |
| unpaid request   |   402    |    **502**    |    402     |       402        |
| forged hash      |   402    |    **502**    |  **502**   |       402        |
| escrow draw      |   402    |    **502**    |    402     |     **402**      |
| authed `/routes` |   200    |    **500**    |  **401**   |       n/a        |

Readiness named the failing dependency precisely in each case — e.g. Redis down
reported `redis: down` with the real reason (`Reached the max retries per
request limit (which is 3)`), and the Soroban-RPC-down gateway rejected the
escrow draw with `Escrow payment failed: Could not read escrow balance` rather
than treating an unknown balance as sufficient. **No dependency outage produced
a `200`**, and auth refused to mint a session without Redis.

> **Finding (open, diagnostics only).** Under Postgres/Redis outage the client
> typed `502 Upstream LLM request failed` — that string is the proxy's generic
> catch-all (`proxy.controller.ts`), not evidence the LLM was contacted. The
> request was correctly refused before forwarding, but the message misattributes
> an infrastructure failure to the upstream provider, which would send an
> operator to the wrong dependency. Recorded here rather than papered over with a
> fragile error-classification guess.
...
**Re-run 2026-09-15 13:21 UTC (this audit), `bash scripts/testnet-journey.sh` → ALL CHECKS PASSED** — a
fresh pass with fresh accounts, quoted so the claim does not rest on a single
historical run. This is the run recorded in `docs/evidence/testnet-journey.json`:

| Fact                        | Value                                                                                                                                                |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Unpaid request              | `402` + quote, amount `1000000` stroops, asset `USDC`, issuer echoed                                                                                 |
| Payment tx                  | `fbf2f72d0d6db5e70c12fba4d50a7299e1f1b259dad0d0e5fd15b140e978c48a`, ledger `4690982`, `0.1 USDC` → `GCTDL5LW…`                                       |
| Retry with `X-Payment-Hash` | `200` + `X-Payment-Receipt`; `receipt.txHash` == the payment hash, `receipt.status` == `confirmed`                                                   |
| Replay                      | `402` — _"This payment has already been used. A new payment is required."_                                                                           |
| Forged hash                 | `402` — _"Payment verification failed: Transaction not found on chain"_                                                                              |
| Receiver balance            | `0.1 USDC`                                                                                                                                           |
| Payout leg (#40)            | `POST /admin/payouts/propose` → auto-approved and **executed**, proposal `8` `executed: true`, `1 USDC` to `GCF7YNP4…`, multisig SAC `85.00 → 84.00` |

Every on-chain transaction from that run (trustline ×2, mint, payment) was
re-checked against Horizon while writing this document — `successful: true` at
ledgers `4690979`–`4690982` — as were all eight hashes quoted elsewhere in this
file.
...
> **Provenance:** the escrow numbers below are from the escrow leg's own run
> (charge/refund transactions closed 10:18 UTC, persistence audit 10:27 UTC, file
> written 10:30 UTC) and are stored in `docs/evidence/testnet-journey.json` under
> `escrow`, which the 13:21 journey re-run deliberately preserved. They were **re-verified
> against Horizon during the 2026-09-15 re-audit** (every hash below is
> `successful: true` at the ledger quoted), but they were **not regenerated** in
> that pass: `scripts/testnet-escrow.sh` deploys a fresh contract and therefore
> needs `cargo`, which is not installed in the audit environment. Re-run
> `bash scripts/testnet-escrow.sh` to reproduce them. Running the journey no
> longer deletes them (defect #19 in §8).
...
3. **Postgres, Redis and Soroban-RPC outages are now injected against a live
   stack** (§5) — all four safety invariants held, and the sweep found a real
   reconnect bug (#17). Still unit-test-only: **LLM-provider failure** and
   **network interruption** (the sweep always reaches a real upstream), and
   there is no chaos run against a **deployed** instance — only the local one.
...
7. **There is no email notification channel** (audited 2026-09-15). `#33` is
   closed on GitHub and `WAVE8_ISSUES.md` used to mark its acceptance criteria as
   met, but no `nodemailer` dependency, handler or `EMAIL_*`/`SMTP_*` config
   exists in the tree — the code was removed as dead code. In-app notifications
   (Postgres-backed) and signed webhooks do exist; see `README.md` and
   `GRANT_SUBMISSION.md`, both corrected in this pass.
8. **The SSRF guard does not pin the address it validated.** The hostname is
   resolved and checked, then handed to `fetch`, which resolves again — a
   rebinding window the 60 s cache narrows but does not close. The IPv6
   classification hole that made internal targets reachable _without_ rebinding
   was fixed (§8 #18); the TOCTOU itself is recorded in `SECURITY.md`
   "Known Residual Risks" #10 and is a follow-up, not a claim.
9. **The Soroban contract suites were not re-executed in this environment** (no
   Rust toolchain): their code paths were read and the controls verified present,
   but the 111-test figure is the 2026-09-14 run. CI runs them on every push.
