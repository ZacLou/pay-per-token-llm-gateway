...
- The **credit-escrow contract** receives and holds USDC on behalf of users.
  ...
  (see §6 and the credit-escrow settlement row in §5; the settlement path is
  implemented and tested but remains off by default).
...
| Credit-escrow settlement                                                                       | ⚠️ Opt-in (default `false`), fire-and-forget by design, gated on `ESCROW_SETTLEMENT_ENABLED` + `CONTRACT_ADMIN_SECRET`. Settles **escrow-funded draws only** (`X-Escrow-User`, recorded as `escrow:<quoteId>`): charges the actual cost for flat-rate and per-token routes and refunds the unused deposit. A Horizon-paid request is never charged against escrow as well (no double billing). The single settlement call site plus the contract's per-`(user, quoteId)` idempotency guards make retries safe. Implemented + unit/e2e tested (#25; hardened 2026-09-13). Keep disabled for mainnet v1 or make it a product decision.                                                                                                                   |
...
| Rate limiting                                                                                  | Unpaid requests are limited per client IP; the paid tier is keyed by the server-verified payer wallet. `TRUST_PROXY` is disabled by default and must be set explicitly behind a reverse proxy, so forged `X-Forwarded-For` cannot widen the unpaid tier on a directly-exposed gateway. Shared-NAT unpaid spam remains a documented residual.                                                                                                                                                                                                                                                                                                                                                                                                           |
| External audit                                                                                 | ❌ Not done (see §1).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Atomic contract initialization (first-`init` front-run)                                        | ✅ **Done 2026-09-14.** Initialization is a Soroban `__constructor`, which runs inside the deploy transaction, and the `init` entry point was removed from all three contracts — so there is no deploy→init gap to race and no post-deploy initialization path at all. `require_auth()` on `init` would not have fixed this (the attacker signs their own address); the constructor does. `scripts/deploy-contracts.sh` now passes constructor args to `stellar contract deploy`. This **changes the deploy ABI**, so the mainnet deployment must supply the constructor arguments (the script does), and any earlier testnet deployment is superseded. All 111 contract tests pass on the new layout (C11 in [`THREAT-MODEL.md`](./THREAT-MODEL.md)). |
...
- [x] Contract initialization made atomic (Soroban `__constructor`, so
      `init` cannot be front-run between deploy and initialization). Done
      2026-09-14; see §5 and THREAT-MODEL C11. The mainnet deploy must pass
      constructor arguments — `scripts/deploy-contracts.sh` does.
